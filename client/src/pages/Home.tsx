...
import { reportConfidenceLabel } from "../../../shared/discovery";
...
function ReportView({ report, username, profileUrl, sourceSignals }: { report: Report; username?: string | null; profileUrl: string; sourceSignals?: any }) {
  ...
  const confidenceLabel = reportConfidenceLabel(sourceSignals ?? {});
  const evidenceConfidence = confidenceLabel.startsWith("Insufficient") ? "Low" : confidenceLabel.split(" ")[0];
  ...
  return <div className="report-shell">
    ...
    <section className="report-hero"><div><span className="label">Overall signal</span><div className="overall-score">{average}<span>/100</span></div><div className={`confidence-badge ${evidenceConfidence.toLowerCase()}`}>{confidenceLabel}</div><p>How clearly the profile communicates, builds trust, and moves visitors toward action.</p></div><div className="hero-meta"><div><span className="label">Category</span><strong>{report.businessCategory}</strong></div><div><span className="label">Personality</span><strong>{report.brandPersonality.join(" · ")}</strong></div></div></section>
...
export default function Home() {
  ...
  const [activeReport, setActiveReport] = useState<{ report: Report; username?: string | null; profileUrl: string; sourceSignals?: any } | null>(null);
  const createAnalysis = trpc.analysis.create.useMutation({ onSuccess: (result) => { const analysis = result.analysis as any; setActiveReport({ report: analysis.report, username: analysis.username, profileUrl: analysis.profileUrl, sourceSignals: analysis.sourceSignals }); } });
