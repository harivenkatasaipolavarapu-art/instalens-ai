...
export default function Agency() {
  ...
  return <main className="app-frame agency-page">
    ...
    <section className="agency-section discovery-section"><div className="section-heading"><div><p className="eyebrow">00 / Top Profiles Discovery</p><h2>Find the strongest profiles in a defined scope</h2></div><span className="section-note">Top 10 / Top 50 in dataset</span></div><p className="discovery-note">Define the market first. Then add a permitted profile list from your approved data source. Results are ranked only within that dataset.</p><div className="provider-empty"><div><b>Automatic discovery is not connected</b><span>Connect an approved Instagram/Meta or business-directory provider to search automatically.</span></div><Button size="sm" variant="outline" onClick={() => setFeedback("Automatic discovery requires an approved provider. The current flow ranks a supplied public-profile dataset.")}>Provider details</Button></div><div className="discovery-form"><label><span>Business category</span><Input value={discoveryCategory} onChange={e => setDiscoveryCategory(e.target.value)} placeholder="e.g. bakeries"/></label><label><span>Location</span><Input value={discoveryLocation} onChange={e => setDiscoveryLocation(e.target.value)} placeholder="e.g. Mumbai"/></label></div><label className="dataset-field"><span>Approved profile dataset <small>optional until a provider is connected</small></span><textarea value={discoveryUrls} onChange={e => setDiscoveryUrls(e.target.value)} placeholder="Paste public Instagram profile URLs, one per line (up to 50)"/></label><div className="discovery-actions"><Button onClick={runDiscovery} disabled={create.isPending}>{create.isPending ? <Loader2 className="spin"/> : <Trophy size={16}/>} {create.isPending ? "Ranking…" : "Rank profiles"}</Button><span>Uses the same five Business DNA dimensions</span></div>{discoveryRows.length > 0 && <div className="discovery-results"><div className="rank-toggle discovery-toggle"><Button size="sm" variant={discoveryLimit === 10 ? "default" : "outline"} onClick={() => setDiscoveryLimit(10)}>Top 10</Button><Button size="sm" variant={discoveryLimit === 50 ? "default" : "outline"} onClick={() => setDiscoveryLimit(50)}>Top 50</Button><span className="muted">{discoveryCategory} · {discoveryLocation}</span><label className="filter-label">Min score <input type="number" min="0" max="100" value={minDiscoveryScore} onChange={e => setMinDiscoveryScore(Number(e.target.value) || 0)}/></label><label className="filter-label">Confidence <select value={confidenceFilter} onChange={e => setConfidenceFilter(e.target.value as any)}><option>All</option><option>High</option><option>Medium</option><option>Low</option></select></label><Button size="sm" variant="outline" onClick={exportDiscoveryCsv}><FileDown size={14}/> CSV</Button></div><div className="discovery-header"><span>Rank</span><span>Profile</span>{dimensions.map(d => <span key={d}>{d}</span>)}<span>Signal</span></div>{filteredDiscoveryRows.slice(0, discoveryLimit).map((row, index) => <div className="discovery-item" key={row.id}><button type="button" className="discovery-row" onClick={() => setExpandedDiscoveryId(expandedDiscoveryId === row.id ? null : row.id)}><strong>#{index + 1}</strong><span>@{row.username ?? "profile"}</span>{scoreKeys.map(key => <b key={key}>{row.report?.scores?.[key]?.score ?? 0}</b>)}<em>{row.average}/100 · {getEvidenceConfidence(row)}</em></button>{expandedDiscoveryId === row.id && <div className="discovery-rationale"><b>Why this score?</b><p>{row.report?.scores?.clarity?.explanation}</p><p>{row.report?.scores?.conversionReadiness?.explanation}</p></div>}</div>)}</div>}</section>
